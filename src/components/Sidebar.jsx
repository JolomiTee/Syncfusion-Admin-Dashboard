            ...
            {activeMenu && (<>
                <div className="flex justify-between items-center">
                    <Link to="/" className='items-center gap-3 ml-3 mt-4 flex text-xl font-extrabold tracking-tight dark:text-white text-slate-900' onClick={handleCloseSidebar}>
                        ...
                        {/* //? What is the diffference between onClick={() => handleCloseSidebar} and onClick={handleCloseSidebar} */}

                        <SiShopware /> <span>GetJoloo</span>
                    </Link>
                    <TooltipComponent content="Menu" position='BottomCenter'>
                        <button type='button' className='text-xl rounded-full p-3 hover:bg-light-gray mt-4 block md:hidden' onClick={() => setActiveMenu((prevMenuState) => !prevMenuState)}>
                            <MdOutlineCancel />
                        </button>
                    </TooltipComponent>
                </div>
                    ...
                    {links.map((item, index) => (
                        <div key={index}>
                            ...
                            {item.links.map((link, index) => (
                                <NavLink key={link.name} to={`/${link.name}`} onClick={handleCloseSidebar} className={({ isActive }) => isActive ? activeLink : inActiveLink}>
                                    {link.icon}
